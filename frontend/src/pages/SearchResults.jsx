...
export default function SearchResults() {
  ...
  // Fetch filtered videos based on the URL query string
  const { data: searchResults = [], isLoading, isError, error } = useQuery({
    queryKey: ["searchVideos", queryTerm],
    queryFn: async () => {
      if (!queryTerm.trim()) return [];
      // Hits your backend's getAllVideos endpoint: GET /api/v1/videos?query=xyz
      const response = await apiClient.get(`/videos?query=${encodeURIComponent(queryTerm)}`);
  ...
  if (isError) {
    return (
      <div className="p-4 bg-rose-950/20 border border-rose-900/40 rounded-xl text-rose-400 text-xs text-center max-w-xl mx-auto mt-6">
        Error loading search results: {error.message || "Network error."}
      </div>
    );
  }
    ...
    <div className="space-y-6 text-white max-w-5xl mx-auto p-2">
      <div className="border-b border-slate-800/60 pb-4">
        <h2 className="text-xl font-black tracking-tight flex items-center gap-2">
          Search Results for: <span className="text-indigo-400">"{queryTerm}"</span>
        </h2>
        <p className="text-[11px] text-slate-400 mt-1 font-medium">
          Found {searchResults.length} matching streams.
        </p>
      </div>
      ...
      {searchResults.length === 0 ? (
        <div className="text-center py-16 bg-slate-900/10 border border-dashed border-slate-800/40 rounded-2xl max-w-xl mx-auto">
          <p className="text-xs text-slate-500 font-medium italic">No matches found for your search criteria.</p>
          <button 
        ...
        <div className="space-y-3 max-w-4xl">
          {searchResults.map((video) => {
            ...
            return (
              <div 
                key={video._id}
                className="group flex flex-col sm:flex-row gap-5 p-3 rounded-2xl bg-[#0d0e15]/40 border border-slate-900/40 hover:bg-[#121420]/60 hover:border-indigo-500/20 transition-all duration-200 shadow-sm"
              >
                {/* Thumbnail */}
                <div 
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5 space-y-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-slate-500 font-semibold">
                      {video.views || 0} views &bull; {video.category}
                    </p>
                    {video.description && (
                      <p className="text-[11px] text-slate-400 line-clamp-2 pt-1 font-medium">
                        {video.description}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <Link 
                      ...
                      className="w-6 h-6 rounded-full bg-slate-800 overflow-hidden border border-slate-700/40 flex items-center justify-center shrink-0"
                    >
                      {video.owner?.avatar ? (
