...
export default function Playlists() {
  ...
  // Query A: Fetch all system videos this user has liked
  ...
  // Query B: Fetch user's custom created playlists
    ...
    <div className="space-y-10 text-white max-w-6xl mx-auto p-2">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          >
            <div className="aspect-video w-full rounded-xl bg-gradient-to-br from-rose-600/20 to-indigo-950 overflow-hidden relative border border-slate-800/40 shadow-inner flex items-center justify-center">
              {likedVideos[0]?.thumbnailUrl ? (
                <img src={likedVideos[0].thumbnailUrl} alt="Liked Thumbnail" className="w-full h-full object-cover opacity-40 blur-[1px] absolute group-hover:scale-102 transition-transform duration-300" />
              ) : null}
          ...
          {/* CARD B: WATCH LATER (DYNAMIC PLACEHOLDER SETUP) */}
          <div className="group flex flex-col bg-[#0d0e15]/40 border border-slate-900/40 p-3 rounded-2xl cursor-not-allowed opacity-60 shadow-sm">
            <div className="aspect-video w-full rounded-xl bg-gradient-to-br from-slate-800/30 to-indigo-950 overflow-hidden relative border border-slate-800/40 flex items-center justify-center">
              <div className="text-center space-y-1">
                <div className="w-10 h-10 rounded-full bg-slate-800 border border-slate-700/50 flex items-center justify-center mx-auto">
                  <svg fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" className="w-5 h-5 text-slate-400">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                  </svg>
                </div>
                <span className="text-xs font-black tracking-wide block pt-1 text-slate-400">Watch Later</span>
              </div>
            </div>
            <div className="pt-2 px-1 flex justify-between items-center text-[10px] text-slate-600 font-semibold">
      ...
      {/* ================= SECTION 2: CUSTOM CREATED PLAYLISTS ================= */}
